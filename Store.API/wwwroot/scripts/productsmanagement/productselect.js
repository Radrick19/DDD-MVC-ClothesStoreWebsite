...
(document).addEventListener("DOMContentLoaded", function () {
    ...
    let subcategoriesSelect = $('#subcategory-form-control').select2({
        ...
        ajax: {
            transport: (data, success, failure) => {
                $.ajax({
                    type: 'GET',
                    url: '/Category/GetAllSubcategories?categoryId=' + $("#categry-form-control").val(),
    ...
    let categorySelect = $('#categry-form-control').select2({
        width: '100%',
        ajax: {
            transport: (data, success, failure) => {
                $.ajax({
    ...
    if (typeof selectedCategoryId != 'undefined' && typeof selectedSubcategoryId != 'undefined') {
        var categoryOpt = new Option(selectedCategoryName, selectedCategoryId, true, true);
        categorySelect.append(categoryOpt);

        var subcategoryOpt = new Option(selectedSubcategoryName, selectedSubcategoryId, true, true);
...
function formatSubcategoryState(state) {
    if (typeof selectedSubcategoryId == 'undefined' || subcategoryChanged == true) {
        return state.displayName;
    }
    else {
