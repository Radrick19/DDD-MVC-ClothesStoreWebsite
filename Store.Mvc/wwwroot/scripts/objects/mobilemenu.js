class MobileMenu {
	...
	constructor() {
		...
		this.CategorySelectionDiv = this.MobileMenuHandler.querySelector('.category-selection');
		this.SearchTypeSelectionDiv = this.MobileMenuHandler.querySelector('.search-type-selection');
		this.FilterSelectionDiv = this.MobileMenuHandler.querySelector('.filter-selection');
		this.PreviousPageButton = this.MobileMenuHandler.querySelector('.backbutton');
		this.BackbuttonAndInfoDiv = this.MobileMenuHandler.querySelector('.backbutton-and-info');
		this.NavigationCategoryName = this.MobileMenuHandler.querySelector('.selected-category-name');
		this.NavigationSearchTypeName = this.MobileMenuHandler.querySelector('.selected-search-type-name');
		this.CategorySelectionDiv.style.display = 'flex';
		for (const categorySelect of this.MobileMenuHandler.querySelectorAll('.select-category')) {
			categorySelect.addEventListener('click', function () {
				self.NavigationCategoryName.innerHTML = categorySelect.innerHTML;
				self.SelectCategory(categorySelect.id);
			})
		}
		for (const searchTypeSelect of this.MobileMenuHandler.querySelectorAll('.select-search-type')) {
	...
	UpdatePreviousButtonValues(windowType) {
		let self = this;
		this.PreviousPageButton.replaceWith(this.PreviousPageButton.cloneNode(true));
		this.PreviousPageButton = this.MobileMenuHandler.querySelector('.backbutton');
